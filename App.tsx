                ...
                {adminActiveTab === 'tournament' && (
                  <div className="bg-white p-8 rounded-3xl shadow-lg space-y-6">
                    ...
                    <div className="flex items-center gap-4 pt-4">
                       ...
                       <button onClick={() => tourneyInputRef.current?.click()} className="px-6 py-3 bg-slate-100 rounded-xl font-bold text-xs uppercase flex items-center gap-2"><FileText size={16}/> {tourneyPdfFile ? tourneyPdfFile.name : 'Carica PDF Regolamento'}</button>
                    </div>
                    <button onClick={handleSaveTournament} disabled={actionLoading} className="w-full py-4 bg-orange-950 text-amber-500 rounded-xl font-black uppercase text-[10px]">AGGIORNA DOCUMENTI</button>
                  </div>
                )}
                  ...
                  <div className="bg-white p-8 rounded-[40px] shadow-xl space-y-8">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-4">
                           ...
                           <textarea value={marqueeInput} onChange={e => setMarqueeInput(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl text-xs font-bold" rows={3} placeholder="News a scorrimento..." />
                        </div>
                     </div>
            ...
            <header className="min-h-screen bg-[#1a0702] relative flex flex-col items-center justify-center p-6 text-center">
              ...
              <div className="relative z-10 space-y-4 animate-fade-in-up">
                ...
                <div className="pt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
                  ...
                  {!isAppInstalled && <button onClick={handleInstallAction} className="px-10 py-4 bg-white/10 text-amber-500 border-2 border-amber-500/50 rounded-2xl font-black uppercase text-xs tracking-widest shadow-2xl transition-all flex items-center gap-2">{deferredPrompt ? <Download size={18} /> : <Info size={18} />} {deferredPrompt ? 'INSTALLA APP' : 'GUIDA APP'}</button>}
                </div>
              </div>
              <button onClick={() => document.getElementById('pwa-guide')?.scrollIntoView({ behavior: 'smooth' })} className="absolute bottom-10 animate-bounce text-amber-500/50"><ArrowDown size={32} /></button>
            </header>
                  ...
                  <div className="max-w-5xl mx-auto space-y-12 text-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      <div className="bg-white p-10 rounded-[40px] shadow-xl space-y-4">
                        ...
                        <h3 className="font-black uppercase italic text-orange-950 text-sm">1. Clicca Installa</h3>
                        <p className="text-[10px] text-slate-400 font-medium leading-relaxed uppercase">Usa il tasto INSTALLA in alto o il menu del browser.</p>
                      </div>
                      <div className="bg-white p-10 rounded-[40px] shadow-xl space-y-4">
                        <div className="w-16 h-16 bg-orange-100 rounded-2xl flex items-center justify-center text-orange-600 mx-auto"><CheckCircle size={32} /></div>
                        <h3 className="font-black uppercase italic text-orange-950 text-sm">2. Aggiungi a Home</h3>
                        <p className="text-[10px] text-slate-400 font-medium leading-relaxed uppercase">Seleziona "Aggiungi a schermata Home" dalle opzioni.</p>
                      </div>
                      <div className="bg-white p-10 rounded-[40px] shadow-xl space-y-4">
                        <div className="w-16 h-16 bg-orange-100 rounded-2xl flex items-center justify-center text-orange-600 mx-auto"><Smartphone size={32} /></div>
                        <h3 className="font-black uppercase italic text-orange-950 text-sm">3. Gioca Subito</h3>
                        <p className="text-[10px] text-slate-400 font-medium leading-relaxed uppercase">L'icona apparirà sul tuo telefono per accesso immediato!</p>
                      </div>
                    </div>
                  </div>
                </section>
            ...
            <main className="max-w-6xl mx-auto p-4 sm:p-8 mb-24 lg:mb-0">
              {activeTab === 'home' && (
                <div className="space-y-8 fade-in">
                   ...
                   <div className="pt-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                     {sponsors.map(s => (
